...
function multiplyBy(numArr, multiplier) {
  ...
  numArr.forEach((element) => {
    multiplied.push(element * multiplier)
  })
...
function filterOut(wordArr, wordsToRemove) {
  ...
  if (!wordsToRemove.length) return wordArr

  const newArr = []

  wordArr.forEach((element) => {
    if (!wordsToRemove.includes(element)) {
      newArr.push(element)
    }
  })
...
function greatestProduct(matrix) {
  ...
  // Check horizontal products (4 consecutive numbers in a row)
  ...
  // Check vertical products (4 consecutive numbers in a column)
